...
fn is_special_form_name(name: &str) -> bool {
    matches!(
        name,
        "if" | "let" | "let-mut" | "while" | "for" | "cond" | "try" | "match"
    )
}
    ...
    if let ExprInner::Call(op, args) = &expr.inner {
        if call_operator_name(op) == Some("if".into()) && !args.is_empty() {
            ...
            let then_ = if args.len() > 1 {
                ...
                Box::new(Expr {
                    span: Span::default(),
                    inner: ExprInner::Atom(AstAtom::Int(0)),
                })
            };
...
fn do_match(pattern: &Expr, expr: &Expr, bindings: &mut IndexMap<String, Expr>) -> Result<(), ()> {
    ...
    match (&pat.inner, &exp.inner) {
        ...
        (ExprInner::Call(op, pats), ExprInner::Call(eop, eargs)) => {
            ...
            if pats.len() != eargs.len() {
                return Err(());
            }
            for (pp, ep) in pats.iter().zip(eargs.iter()) {
                do_match(pp, ep, bindings)?;
            }
            Ok(())
        }
        ...
        (ExprInner::Apply(pname, pargs), ExprInner::Apply(ename, eargs)) if pname == ename => {
            ...
            for (pp, ep) in pargs.iter().zip(eargs.iter()) {
                do_match(pp, ep, bindings)?;
            }
            Ok(())
        }
        ...
        (ExprInner::Begin(pexprs), ExprInner::Begin(eexprs)) => {
            if pexprs.len() != eexprs.len() {
                return Err(());
            }
            for (pp, ep) in pexprs.iter().zip(eexprs.iter()) {
                do_match(pp, ep, bindings)?;
            }
            Ok(())
        }
        ...
        (ExprInner::If(pc, pt, pe), ExprInner::If(ec, et, ee)) => {
            do_match(pc, ec, bindings)?;
            do_match(pt, et, bindings)?;
            do_match(pe, ee, bindings)
        }
        // For other complex types: try structural equality.
        _ => {
            if expr_matches_structurally(&pat, &exp) {
                Ok(())
            } else {
                Err(())
            }
...
fn expr_matches_structurally(a: &Expr, b: &Expr) -> bool {
    use ExprInner::*;
    std::mem::discriminant(&a.inner) == std::mem::discriminant(&b.inner)
        && match (&a.inner, &b.inner) {
            (Atom(aa), Atom(ab)) => aa == ab,
            (Call(aop, aargs), Call(bop, bargs)) => {
                expr_matches_structurally(aop, bop)
                    && aargs.len() == bargs.len()
                    && aargs
                        .iter()
                        .zip(bargs.iter())
                        .all(|(x, y)| expr_matches_structurally(x, y))
            }
            (Def(an, av), Def(bn, bv)) => an == bn.as_str() && expr_matches_structurally(av, bv),
            (Let(al, av, ab), Let(bl, bv, bb)) => {
                al == bl && expr_matches_structurally(av, bv) && expr_matches_structurally(ab, bb)
            }
            (If(ac, at, ae), If(bc, bt, be)) => {
                expr_matches_structurally(ac, bc)
                    && expr_matches_structurally(at, bt)
                    && expr_matches_structurally(ae, be)
            }
            (Begin(ax), Begin(bx)) => {
                ax.len() == bx.len()
                    && ax
...
fn atom_ref(name: String) -> Expr {
    Expr {
        span: Span::default(),
        inner: ExprInner::Atom(Atom::Ident(name)),
    }
}
    ...
    let result = match &expr.inner {
        ...
        ExprInner::Call(op, args) => {
            ...
            if matches!(op_name.as_deref(), Some("if")) && !args.is_empty() {
                ...
                let gensym_name = format!("__let_{}", {
                    let mut g = ctx.gensyms.borrow_mut();
                    g.gensym(&name)
                });
                inner_bindings.insert(name.clone(), atom_ref(gensym_name));
                let inner_ctx = SubstContext {
        ...
        ExprInner::Apply(name, args) => match name.as_str() {
            "if" if !args.is_empty() => {
                let cond = sub_expr(ctx, &args[0]);
                let then_ = if args.len() > 1 {
                    sub_expr(ctx, &args[1])
        ...
        ExprInner::Def(name, val) => ExprInner::Def(name.clone(), Box::new(sub_expr(ctx, val))),
        ...
        ExprInner::Let(name, val, body) => {
            ...
            ExprInner::Let(
                name.clone(),
                Box::new(new_val),
                Box::new(sub_expr(&inner_ctx, body)),
            )
        }
        ...
        ExprInner::If(cond, then_, else_) => ExprInner::If(
            ...
            Box::new(sub_expr(ctx, then_)),
            Box::new(sub_expr(ctx, else_)),
        ),
        ...
        ExprInner::Lambda(_, params, body) | ExprInner::Fn(_, params, body) => {
            let mut inner_bindings = ctx.bindings.clone();
            for p in params {
                let gensym_name = format!("__param_{}", {
                    let mut g = ctx.gensyms.borrow_mut();
                    g.gensym(&p.name)
                });
...
fn sub_complex(ctx: &SubstContext, expr: &Expr) -> ExprInner {
    ...
    match &expr.inner {
        Call(op, args) => {
            let new_op = Box::new(sub_expr(ctx, op));
            let new_args: Vec<Expr> = args.iter().map(|a| sub_expr(ctx, a)).collect();
            Call(new_op, new_args)
        }
        Def(name, val) => Def(name.clone(), Box::new(sub_expr(ctx, val))),
        LetMut(name, val, body) => LetMut(
            name.clone(),
            Box::new(sub_expr(ctx, val)),
            Box::new(sub_expr(ctx, body)),
        ),
        TryCatch(e, name, h) => TryCatch(
            Box::new(sub_expr(ctx, e)),
            name.clone(),
            Box::new(sub_expr(ctx, h)),
        ),
        Match(e, arms) => {
            let new_arms: Vec<_> = arms
                ...
                .map(|arm| MatchArm {
                    ...
                    patterns: arm.patterns.iter().map(|p| sub_expr(ctx, p)).collect(),
                    body: Box::new(sub_expr(ctx, &arm.body)),
                })
                .collect();
        ...
        Cond(clauses) => {
            let new_clauses: Vec<_> = clauses
                .iter()
                .map(|(pred, b)| (Box::new(sub_expr(ctx, pred)), Box::new(sub_expr(ctx, b))))
                .collect();
            Cond(new_clauses)
        }
        StructGet(s, field) => StructGet(Box::new(sub_expr(ctx, s)), field.clone()),
        MakeStruct(name, args) => {
            let new_args: Vec<Expr> = args.iter().map(|a| sub_expr(ctx, a)).collect();
            MakeStruct(name.clone(), new_args)
        }
        ...
        WithResource(name, init, body) => WithResource(
            name.clone(),
        ...
        StructDef(sd) | StructDefPlus(sd) => {
            if matches!(&expr.inner, ExprInner::StructDefPlus(_)) {
                ExprInner::StructDefPlus(sd.clone())
            } else {
                ExprInner::StructDef(sd.clone())
            }
        }
        ...
        TestProperty(name, gen, body) => {
            TestProperty(name.clone(), gen.clone(), Box::new(sub_expr(ctx, body)))
        }
        Setup(exprs) | Teardown(exprs) => {
            if matches!(&expr.inner, ExprInner::Teardown(_)) {
                Teardown(exprs.iter().map(|e| sub_expr(ctx, e)).collect())
            } else {
                Setup(exprs.iter().map(|e| sub_expr(ctx, e)).collect())
            }
        }
        RunTests(keywords) => RunTests(keywords.clone()),
        TestCompile(e, expect_error) => TestCompile(Box::new(sub_expr(ctx, e)), *expect_error),
        MacroDef(name, patterns, template) => {
            let new_patterns: Vec<Expr> = patterns.iter().map(|p| sub_expr(ctx, p)).collect();
            MacroDef(
                name.clone(),
                new_patterns,
                Box::new(sub_expr(ctx, template)),
            )
        }
        ...
        Defn(_, params, body) => {
            ...
            Defn("".into(), params.clone(), Box::new(sub_expr(&ic, body)))
        }
        Let(name, val, body) => {
            let nv = sub_expr(ctx, val);
            let mut ib = ctx.bindings.clone();
            ib.insert(
                name.clone(),
                atom_ref(format!("__let_{}", {
                    let mut g = ctx.gensyms.borrow_mut();
                    g.gensym(name)
                })),
            );
            ...
            Let(name.clone(), Box::new(nv), Box::new(sub_expr(&ic, body)))
        }
        If(c, t, e_) => If(
            Box::new(sub_expr(ctx, c)),
            Box::new(sub_expr(ctx, t)),
            Box::new(sub_expr(ctx, e_)),
        ),
        Begin(exprs) => Begin(exprs.iter().map(|e| sub_expr(ctx, e)).collect()),
        Lambda(_, params, body) | Fn(_, params, body) => {
            let mut ib = ctx.bindings.clone();
            for p in params {
                ib.insert(
                    p.name.clone(),
            ...
            if matches!(&expr.inner, ExprInner::Lambda(..)) {
                Lambda("".into(), params.clone(), Box::new(sub_expr(&ic, body)))
            } else {
                Fn("".into(), params.clone(), Box::new(sub_expr(&ic, body)))
            }
        }
...
fn try_extract_macro_def(expr: &Expr) -> Option<MacroDef> {
    match &expr.inner {
        // Dispatched form.
        ExprInner::MacroDef(name, patterns, template) => Some(MacroDef {
            name: name.clone(),
        ...
        ExprInner::Call(op, args)
            if call_operator_name(op) == Some("defmacro".into()) && args.len() >= 3 =>
        {
            let name = match &args[0].inner {
                ExprInner::Atom(Atom::Ident(n)) => n.clone(),
                _ => return None,
...
impl MacroExpander {
    ...
    fn expand_expr(&mut self, mut expr: Expr) -> Result<Expr, ZylError> {
        ...
        match &expr.inner {
            ExprInner::Call(op, args) => {
                ...
                for arg in args {
                    new_args.push(self.expand_expr(arg.clone())?);
                }
            ...
            ExprInner::LetMut(_, val, body) => {
                let name = match &expr.inner {
                    ExprInner::LetMut(n, _, _) => n.clone(),
                    _ => unreachable!(),
                };
                expr.inner = ExprInner::LetMut(
                    name,
                    Box::new(self.expand_expr(*val.clone())?),
                    ...
                    .map(|arm| {
                        Ok(MatchArm {
                            ...
                            patterns: arm
                                .patterns
                                .iter()
                                .map(|p| self.expand_expr(p.clone()))
                                .collect::<Result<Vec<_>, _>>()?,
                            body: Box::new(self.expand_expr(*arm.body.clone())?),
                        })
                    })
                    .collect::<Result<_, _>>()?;
            ...
            ExprInner::Spawn(e) => {
                expr.inner = ExprInner::Spawn(Box::new(self.expand_expr(*e.clone())?));
            }
            ExprInner::Send(a, m) => {
            ...
            ExprInner::FfiPin(e) => {
                expr.inner = ExprInner::FfiPin(Box::new(self.expand_expr(*e.clone())?));
            }
            ExprInner::FfiUnpin(e) => {
                expr.inner = ExprInner::FfiUnpin(Box::new(self.expand_expr(*e.clone())?));
            }
            ExprInner::Assert(c, msg) => {
                let new_msg: Option<Expr> = match msg {
                    Some(m) => Some(self.expand_expr(atom_ref(m.clone()))?),
                    None => None,
                };
                expr.inner = ExprInner::Assert(
                    Box::new(self.expand_expr(*c.clone())?),
                    new_msg.map(|e| match &e.inner {
            ...
            ExprInner::Unwrap(e) => {
                expr.inner = ExprInner::Unwrap(Box::new(self.expand_expr(*e.clone())?));
            }
            ExprInner::While(c, b) => {
            ...
            ExprInner::Cond(clauses) => {
                let new_clauses: Vec<(Box<Expr>, Box<Expr>)> = clauses
                    ...
                    .map(|(pred, b)| {
                        Ok((
                            Box::new(self.expand_expr(*pred.clone())?),
                            Box::new(self.expand_expr(*b.clone())?),
                        ))
                    })
                    .collect::<Result<Vec<_>, _>>()?;
            ...
            ExprInner::If(cond, then_, else_) => {
                expr.inner = ExprInner::If(
                    Box::new(self.expand_expr(*cond.clone())?),
                    Box::new(self.expand_expr(*then_.clone())?),
                    Box::new(self.expand_expr(*else_.clone())?),
                );
            }
            ExprInner::Let(name, val, body) => {
            ...
            ExprInner::Lambda(_, params, body) | ExprInner::Fn(_, params, body) => {
                ...
                if matches!(&expr.inner, ExprInner::Lambda(..)) {
                    expr.inner = ExprInner::Lambda("".into(), params.clone(), new_body);
                } else {
                    expr.inner = ExprInner::Fn("".into(), params.clone(), new_body);
                }
            }
            ExprInner::StructGet(s, field) => {
                let f = field.clone();
                expr.inner = ExprInner::StructGet(Box::new(self.expand_expr(*s.clone())?), f);
            }
            ExprInner::MakeStruct(name, args) => {
                ...
                let new_args: Vec<Expr> = args
                    ...
                    .map(|a| self.expand_expr(a.clone()))
                    .collect::<Result<Vec<_>, _>>()?;
                expr.inner = ExprInner::MakeStruct(mname, new_args);
            }
            ExprInner::SetBang(name, val) => {
                expr.inner =
                    ExprInner::SetBang(name.clone(), Box::new(self.expand_expr(*val.clone())?));
            }
            ...
            ExprInner::WithResource(name, init, body) => {
                ...
                expr.inner = ExprInner::WithResource(
                    rname,
                    Box::new(self.expand_expr(*init.clone())?),
                    Box::new(self.expand_expr(*body.clone())?),
                );
            }
            ...
            _ => {
                ...
                let new_inner = match &expr.inner {
                    Def(_, val) => {
                        ...
                        Def(name, Box::new(self.expand_expr(*val.clone())?))
                    }
                    LetMut(_, val, body) => {
                        let name = match &expr.inner {
                            LetMut(n, _, _) => n.clone(),
                            _ => unreachable!(),
                        };
                        LetMut(
                            name,
                    ...
                    Match(e, arms) => {
                        let new_arms: Vec<MatchArm> = arms
                            ...
                            .collect::<Result<_, _>>()?;
                        Match(Box::new(self.expand_expr(*e.clone())?), new_arms)
                    }
                    ...
                    Assert(c, msg) => {
                        let nm: Option<String> = match msg {
                            Some(m) => {
                                Some(self.expand_expr(atom_ref(m.clone())).map(
                                    |e| match &e.inner {
                                        ExprInner::Atom(AstAtom::Str(s)) => s.clone(),
                                        _ => unreachable!(),
                                    },
                                )?)
                            }
                        ...
                        Assert(Box::new(self.expand_expr(*c.clone())?), nm)
                    }
                    ...
                    Cond(clauses) => {
                        let nc: Vec<(Box<Expr>, Box<Expr>)> = clauses
                            ...
                            .map(|(p, b)| {
                                Ok((
                                    Box::new(self.expand_expr(*p.clone())?),
                                    Box::new(self.expand_expr(*b.clone())?),
                                ))
                            })
                            .collect::<Result<Vec<_>, _>>()?;
                    ...
                    StructGet(s, field) => {
                        let f = field.clone();
                        StructGet(Box::new(self.expand_expr(*s.clone())?), f)
                    }
                    MakeStruct(name, args) => {
                        let mn = name.clone();
                        let na: Vec<Expr> = args
                    ...
                    SetBang(name, val) => {
                        SetBang(name.clone(), Box::new(self.expand_expr(*val.clone())?))
                    }
                    Print(exprs) => {
                    ...
                    AliasDecl(name, target) => {
                        AliasDecl(name.clone(), Box::new(self.expand_expr(*target.clone())?))
                    }
                    TestCompile(e, expect_error) => {
                    ...
                    _ => {
                        ...
                        match &expr.inner {
                            Call(op, args) => {
                                let no = Box::new(self.expand_expr(*op.clone())?);
                                let na: Vec<Expr> = args
                                    .iter()
                                    .map(|a| self.expand_expr(a.clone()))
                                    .collect::<Result<Vec<_>, _>>()?;
                                ExprInner::Call(no, na)
                            }
                            Apply(name, args) => {
                                let mut na: Vec<Expr> = Vec::with_capacity(args.len());
                                for a in args {
                                    na.push(self.expand_expr(a.clone())?);
                                }
                                ExprInner::Apply(name.clone(), na)
                            }
    ...
    fn try_expand(&mut self, mut expr: Expr) -> Result<Expr, ZylError> {
        let (name, args) = match &expr.inner {
            ...
            ExprInner::Apply(name, args) => {
                if !self.macros.contains_key(name.as_str()) && !is_macro_candidate(name) {
                    return Ok(expr);
                }
...
fn has_variadic_pattern(patterns: &[Expr]) -> bool {
    patterns
        .iter()
        .any(|p| matches!(&p.inner, ExprInner::Atom(Atom::Ident(n)) if n.starts_with("&")))
}
...
fn is_macro_candidate(name: &str) -> bool {
    let excluded = [
        "+", "-", "*", "/", "%", "==", "!=", "<", ">", "<=", ">=", "not", "and", "or", "len",
        "vec", "tuple", "map", "identity", "compose",
    ];
