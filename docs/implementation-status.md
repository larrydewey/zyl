...
| Feature | Status |
|---|---|
| S-expression syntax, dispatch-free reader | Works. `[a b]` reads as `(list a b)`, `'d` as `(quote d)`, `` `d `` as `(quasiquote d)`, `,e` as `(unquote e)` and `,@e` as `(unquote-splicing e)`; `&` may start an identifier |
| List literals | Works: `(list a b c)`, `[a b c]` and quoted constant data `'(1 2 3)` are `Cons` chains built in source order (left-to-right evaluation, one element type, `E_TYPE_MISMATCH` otherwise); `[]`, `(list)` and `'()` are `Nil`. Quoted data holds Int, Float, String, Bool and nested lists; a name inside it is `E_MALFORMED_FORM` (there is no symbol type). A derive's `[Eq Show]` still names traits. Quasiquote works: `` `(1 ,x ,@ys) `` is `(Cons 1 (Cons x (zyl-qq-append ys Nil)))`, one element type; a name outside an unquote and a nested quasiquote are `E_MALFORMED_FORM` |
...
| Macros | `defmacro` expansion works in every form, with gensym hygiene, arity, duplicate and termination checks; parameters are plain names (no patterns), and the list may end in `&rest name`, which a template splices with `,@name` where a form takes any number of expressions, or uses as the list of its arguments |
